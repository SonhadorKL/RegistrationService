...
PasswordChecker::PasswordChecker(const std::string &path_to_config)
    : stream_to_json_(path_to_config) {
...
std::string PasswordChecker::Check(const std::string &password) {
  ...
  for (auto &[status, characteristic] : password_requirements_.items()) {
    const std::string &preprocessed_password =
        Preprocess(password, characteristic["special"]);
...
PasswordChecker::~PasswordChecker() { stream_to_json_.close(); }

std::string PasswordChecker::Preprocess(const std::string &password,
                                        int weak_words_count) {
