...
void Client::GetConnection(std::string &user, std::string &password,
                           std::string &host_ip, std::string &port) {
...
bool Client::GetNext(Query &to_request) {
  ...
  if (processing_command.GetCommand() == "/register") {
    ...
    throw std::invalid_argument("Don't know the command " +
                                processing_command.GetCommand() + '\n');
  }
...
void Client::CheckCommandArgs(Command &processing, int arg_count) {
  if (processing.GetArgSize() != arg_count) {
    throw std::invalid_argument(
        "Invalid argument count: " + processing.GetCommand() + " expected " +
        std::to_string(arg_count) + " args - got " +
        std::to_string(processing.GetArgSize()) + " \n");
  }
}
